...
export function TabItem({ tab, isActive }: TabItemProps) {
  ...
  const handleOpenBrowserTab = () => {
    const url = `/workspace/${tab.workspaceId}/${tab.pageId}`;
    window.open(url, "_blank", "noopener,noreferrer");
  };

  const handleCopyLink = async () => {
    try {
      const url = `${window.location.origin}/workspace/${tab.workspaceId}/${tab.pageId}`;
  ...
  return (
    <ContextMenu>
      ...
      >
        ...
        {isActive && (
          <span className="absolute inset-x-2 -bottom-px h-[2px] rounded-full bg-(--accent-blue)" />
        )}
      </ContextMenuTrigger>

      <ContextMenuContent align="start" className="w-52 text-xs">
