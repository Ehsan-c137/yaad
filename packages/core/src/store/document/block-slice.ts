...
> = (set, get) => ({
  ...
  updateBlockTags: async (blockId, pageId, tags) => {
    ...
    await updateAndDebounceSave(get, set, (doc) =>
      applyBlockTags(doc, blockId, tags),
    );
  },

  addTagToBlock: async (blockId, pageId, tag) => {
    const currentDoc = get().currentDocument;
    const block =
      currentDoc?.id === pageId ? currentDoc.blocks[blockId] : undefined;
    const existingTags = block?.tags ?? [];
    if (existingTags.some((t) => t.id === tag.id)) return;

    await get().updateBlockTags(blockId, pageId, [...existingTags, tag]);
  },

  removeTagFromBlock: async (blockId, pageId, tagId) => {
    const currentDoc = get().currentDocument;
    const block =
      currentDoc?.id === pageId ? currentDoc.blocks[blockId] : undefined;
    const existingTags = block?.tags ?? [];

    await get().updateBlockTags(
      blockId,
      pageId,
      existingTags.filter((t) => t.id !== tagId),
    );
  },
  ...
  addBlock: async (parentId, afterBlockId, type) => {
    ...
    if (type === "page") {
      targetPageId = generatePageId();
      const currentDocId = get().currentDocument?.id;

      if (currentDocId) {
        const createdId = await documentService.createSubPage(
          currentDocId,
          "Untitled",
        );
        if (createdId) targetPageId = createdId;
      } else {
    ...
    await updateAndDebounceSave(get, set, (doc) => {
      const newBlock: DocumentBlock = {
        ...
        parentId,
        childrenIds: [],
        properties: createBlockDefaults(type, { targetPageId }),
        tags: [],
        createdAt: Date.now(),
        updatedAt: Date.now(),
      };
...
async function handleTransitionToPage(
  ...
  if (currentDocId) {
    useSidebarStore
      .getState()
      .registerSubPageInTree(newPageId, currentDocId, existingTitle, "📄");
  }
