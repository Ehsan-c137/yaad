...
describe("document block-slice (Unit Test)", () => {
  ...
  it("addTagToBlock adds a tag to the target block", async () => {
    const store = getDocumentStore(pageId);
    const tag = { id: "t1", name: "Urgent", color: "red" as const };

    await store.getState().addTagToBlock("b1", pageId, tag);

    const doc = store.getState().currentDocument!;

    expect(doc.blocks.b1.tags).toEqual([tag]);
  ...
  it("removeTagFromBlock removes a tag from the target block", async () => {
    const store = getDocumentStore(pageId);
    const tag1 = { id: "t1", name: "Urgent", color: "red" as const };
    const tag2 = { id: "t2", name: "Work", color: "blue" as const };

    await store.getState().updateBlockTags("b1", pageId, [tag1, tag2]);
    await store.getState().removeTagFromBlock("b1", pageId, "t1");

    const doc = store.getState().currentDocument!;

    expect(doc.blocks.b1.tags).toEqual([tag2]);
  });
});
